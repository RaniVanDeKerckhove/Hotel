...
public class RegistrationRepository : IRegistrationRepository
{
    ...
    public Registration GetRegistrationById(int registrationId)
    {
        try
        {
            ...
            string sql = "SELECT r.[RegistrationId], r.[CustomerId], r.[ActivityId], " +
                         "r.[TotalPrice], r.[ChildCost], r.[AdultCost], " +
                         "r.[NumberOfAdults], r.[NumberOfChildren], " +
                         "c.[Name] AS CustomerName, a.[Name] AS ActivityName, " +
                         "c.[ContactInfo_Email], " +
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        Activity activity = new Activity(
                            ...
                            duration: reader["Duration"] == DBNull.Value ? 0 : Convert.ToInt32(reader["Duration"]),
                            availablePlaces: reader["AvailablePlaces"] == DBNull.Value
                                ? 0
                                : Convert.ToInt32(reader["AvailablePlaces"]),
                            priceAdult: reader["PriceAdult"] == DBNull.Value
                                ? 0
                                : Convert.ToDecimal(reader["PriceAdult"]),
                            priceChild: reader["PriceChild"] == DBNull.Value
                                ? 0
                                : Convert.ToDecimal(reader["PriceChild"]),
                            discount: reader["Discount"] == DBNull.Value ? 0 : Convert.ToDecimal(reader["Discount"]),
                            location: reader["Location"] == DBNull.Value ? string.Empty : reader["Location"].ToString()
                        );
        ...
        {
            throw new RegistrationRepositoryException("getregistration", ex);
        }
    }
